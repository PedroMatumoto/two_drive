...
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
public class OptionScreenAudio extends JFrame implements ActionListener{
    ...
    public OptionScreenAudio(){
        ...
        messages = ResourceBundle.getBundle("src/messageswav");
        ...
        if(e.getSource() == btnUpload){
            try {
                ...
                if (res == JFileChooser.APPROVE_OPTION) {
                    ...
                    servsock.close();
                    setVisible(false);
                    OptionScreenAudio optionScreenAudio = new OptionScreenAudio();
                }
                }
            ...
            ClienteBatepapo client = new ClienteBatepapo();
            try {
                client.start("downloadwav:" + filename);
            ...
            ClienteBatepapo client = new ClienteBatepapo();
            try {
                client.start("sendallwav");
                String tudoconcatenado = client.getMsg();
                // tira oo all txt
                tudoconcatenado = tudoconcatenado.replace("allwav;", "");
            ...
            ClienteBatepapo client = new ClienteBatepapo();
            try {
                client.start("removerWAV:"+arqname);
                if (client.getMsg().equals("true")) {
                    JOptionPane.showMessageDialog(null, "Wav apagado com sucesso!");
                } else {
                    JOptionPane.showMessageDialog(null, "Wav não encontrado!");
                }
            } catch (Exception e1) {
                e1.printStackTrace();
            }
    ...
    private void updateTranslations(Locale locale) {
        messages = ResourceBundle.getBundle("src.messageswav", locale);
